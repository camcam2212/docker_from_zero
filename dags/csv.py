...
def load_csv_to_postgres():
    ...
    pg_hook = PostgresHook(postgres_conn_id='postgres')  # 'postgres' là Conn Id bạn đã tạo
    ...
    for file_name in os.listdir(CSV_FOLDER):
        if file_name.endswith('.csv'):
            ...
            for index, row in df.iterrows():
                cursor.execute(
                    "INSERT INTO your_table (id, based_date, amount, quantity) VALUES (%s, %s, %s, %s)",
                    (row['id'], row['based_date'], row['amount'], row['quantity'])
                )
